#include <bits/stdc++.h>
#define forr(i, a, n) for(int i = a; i < n; i++)
#define forn(i, n) for(int i = 0; i < n; i++)
#define dfor(i, n) for(int i = n - 1; i >= 0; i--)
#define forall(it, v) for(auto it = v.begin(); it != v.end(); it++)
#define pb push_back
#define sz(a) ((int)a.size())
#define all(x) (x).begin(),(x).end()
#define rall(x) (x).rbegin(),(x).rend()
#define dbg(x) cout << #x << " = " << (x) << endl
#define vdbg(x) {cout << '['; for(auto i : x) cout << i << ", "; cout << "]\n";}
#define fr first
#define sc second

using namespace std;

typedef long long ll;
typedef pair<int, int> ii;

struct UnionFind {
    int nsets;
    vector<int> f, setsz;  // f[i] = parent of node i
    UnionFind(int n) : nsets(n), f(n, -1), setsz(n, 1) {}
    int comp(int x) { return (f[x] == -1 ? x : f[x] = comp(f[x])); }  // O(1)
    bool join(int i, int j) {  // returns true if already in the same set
        int a = comp(i), b = comp(j);
        if (a != b) {
            if (a < b) swap(a, b);
            f[a] = b;  // the bigger group (b) now represents the smaller (a)
            nsets--, setsz[b] += setsz[a];
        }
        return a == b;
    }
};

int main(){
    ios::sync_with_stdio(0);
    cin.tie(0);
    #ifdef fran
        freopen("input.in", "r", stdin);
        freopen("output.out", "w", stdout);
    #endif

    int n; cin >> n; 
    vector<pair<int, ii>> ar;
    vector<vector<bool>> use(n, vector<bool>(n));
    vector<ii> ans;

    int x; 
    forn(i, n) forn(j, n) {
        cin >> x;
        ar.pb({x, {max(i, j), min(i, j)}});
    }

    sort(rall(ar));
    UnionFind uf(n);
    ll sum = 0;
    vector<int> trans(n);
    forn(i, sz(ar)) {
        if(uf.comp(ar[i].sc.fr) != uf.comp(ar[i].sc.sc)) ans.pb({uf.comp(ar[i].sc.fr), uf.comp(ar[i].sc.sc)});
        if(!uf.join(ar[i].sc.fr, ar[i].sc.sc)) sum += ar[i].fr;
    }

    cout << sum << '\n';
    forn(i, sz(ans)) cout << ans[i].fr+1 << ' ' << ans[i].sc+1 << '\n';



    return 0;
}