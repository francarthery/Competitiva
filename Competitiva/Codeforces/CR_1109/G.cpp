...
int main(){
    ...
    while(t--) {
        ...
        vector<int> v(n);
        forn(i, n) cin >> v[i];
        map<int, vector<ii>> pend;
        ST dp(n+1);

        forn(i, n) {
            for(auto j : pend[i]) { //Aplico los valores que ahora son validos
                dp.set(j.fr, j.sc);
            }
            pend.erase(i); //Elimino lo ya aplicado

            pend[i+v[i]+1].pb({i, dp.get(0, i - v[i]) + v[i]});
        }
