...
void menu_principal(){
	...
	switch(opcion){
		...
		case 'X':
			gotoxy(30, 17);
			SetConsoleTextAttribute(GetStdHandle (STD_OUTPUT_HANDLE),244);
			cout << "Est" << (char)160 << " seguro que desea salir de la aplicaci" << (char)162 << "n? [S/N]: ";
			SetConsoleTextAttribute(GetStdHandle (STD_OUTPUT_HANDLE),241);
...
void memoria(string linea[]){
    ...
    gotoxy(20, 6);
	cout << "a-) " << (char)168 << "Cu" << (char)160 << "l es la letra que m" << (char)160 << "s se repit" << (char)161 << "a en las palabras?: ";
