...
//Se utiliza para crear l√≠neas centradas en una fila espec√≠fica de la consola. 
...
bool login() {
	...
	//Se utliza la siguiente funci√≥n para los pedir datos de forma segura.
    ...
    if (contador == 3 and !valido){
        ...
        cout << "Demasiados intentos incorrectos. Pruebe de nuevo m·s tarde" << endl;
...
void ingresodatos(char usuario[], char password[], bool codigo){
    ...
    if(codigo == 1){
        gotoxy(25, 17);
        SetConsoleTextAttribute(GetStdHandle (STD_OUTPUT_HANDLE),244);
        cout << "Usuario o contraseÒa incorrecta. Intente nuevamente" << endl;
        SetConsoleTextAttribute(GetStdHandle (STD_OUTPUT_HANDLE),241);
    }
...
void ingreso_contra(char password[]){
    ...
    while(true){
        ...
        //Si es un enter salimos del bucle y colocamos caracter de fin de l√≠nea
        if(caracter == 13){
            password[contador + 1] = '\0';
            break;
        }
        //Si es retroceso borramos el √∫ltmo caracter
...
bool verificacion_password (int usuario, char contrasena[]) {
    ...
    //Almacenamos el valor que deberia tener la contrase√±a para luego comparar con la ingresada
...
//Funcion recursiva que invierte los digitos de un n√∫mero
...
void menu_principal(){
	...
	switch(opcion){
		...
		case 'X':
			gotoxy(30, 17);
			SetConsoleTextAttribute(GetStdHandle (STD_OUTPUT_HANDLE),244);
			cout << "øEst· seguro que desea salir de la aplicaciÛn? [S/N]: ";
			SetConsoleTextAttribute(GetStdHandle (STD_OUTPUT_HANDLE),241);
			...
			do{
				...
				if(confirmacion == 'S'){
					...
					gotoxy(30, 17);
					SetConsoleTextAttribute(GetStdHandle (STD_OUTPUT_HANDLE),244);
					cout << "Caracter incorrecto. Ingrese confirmaciÛn nuevamente: " << endl;
					SetConsoleTextAttribute(GetStdHandle (STD_OUTPUT_HANDLE),241);
					gotoxy(84, 17);
					cout << "         ";
					gotoxy(84, 17);
		...
		default:
			gotoxy(30, 17);
			SetConsoleTextAttribute(GetStdHandle (STD_OUTPUT_HANDLE),244);
			cout << "La opciÛn ingresada no es correcta" << endl;
			SetConsoleTextAttribute(GetStdHandle (STD_OUTPUT_HANDLE),241);
			Sleep(2000);
			menu_principal();
			break;
...
//Ingresan los datos a un array de caracteres para evitar errores en el caso de que el usuario ingrese mas de un d√≠gito.
char pedir_opciones(){
	...
	//Si el largo del ingreso es mayor a un caracter. Se retorna un caracter '?' el cual no representa ninguna opci√≥n posible.
    if(ingreso.length() != 1){
        rta = '?';
    }
    else{
		//Si el largo es 1, se retorna el caracter que ingres√≥ el usuario.
        rta = ingreso[0];
    }

    return rta;
}
//------------------------------------------------- Men˙ procesador de texto ------------------------------------------------------------
/*void Menu_procesador(){
	bool bandera= true;
	char opcion;
	do{
		lineas(8, 20, 48, '=');
		gotoxy(43, 10);
		cout << "Men˙ procesador de texto";
		lineas(-1, 11, 28, '=');
		gotoxy(36, 12);
		cout << "1.- EstadÌsticas a nivel caracteres";
		gotoxy(36, 13);
		cout << "2.- EstadÌsticas a nivel palabras";
		gotoxy(36, 14);
		...
		switch(opcion){
			...
			case 'X':
				gotoxy(26, 19);
				SetConsoleTextAttribute(GetStdHandle (STD_OUTPUT_HANDLE),244);
				cout << "øEst· seguro que desea volver al men˙ principal? [S/N]: ";
				SetConsoleTextAttribute(GetStdHandle (STD_OUTPUT_HANDLE),241);
				do{
...
void espanol(){
	...
	gotoxy(40, 2);
	cout << "Abecedario espaÒol";
	
	lineas(3, 23, 80, '=');
	gotoxy(10, 4);
	cout << "Letras min˙sculas:";
	letras_minusculas(10, 5);
	
	gotoxy(10, 9);
	cout <<"Letras may˙sculas:";
...
void ingles(){
	...
	gotoxy(41, 2);
	cout << "Abecedario inglÈs";
	
	lineas(3, 16, 80, '=');
	gotoxy(10, 4);
	cout << "Lowercase letters (letras min˙sculas):";
	letras_minusculas(10, 5);
	
	gotoxy(10, 9);
	cout << "Capital letters (letras may˙sculas):";
	letras_mayusculas(10,10);
...
void italiano(){
	...
	lineas(3, 23, 80, '=');
	gotoxy(10, 4);
	cout << "Lettere minuscole (letras min˙sculas): ";
	gotoxy(10, 5);
	for(char i = 'a'; i <= 'z'; i++){
		cout << i << " ";
		contador++;
	...
	gotoxy(10, 9);
	cout << "Lettere maiuscole (letras may˙sculas):" << endl;
...
void portugues(){
	...
	gotoxy(10, 9);
	cout << "Letras maiusculas (letras may˙sculas):";
	letras_mayusculas(10,10);
...
void frances(){
	...
	lineas(3, 26, 80, '=');
	gotoxy(10, 4);
	cout << "Lettres minuscules (letras min˙sculas):";
	letras_minusculas(10,5);
	
	gotoxy(10, 9);
	cout << "Lettres majuscules (letras may˙sculas):";
	letras_mayusculas(10,10);
	...
	gotoxy(10, 23);
	cout << "Lettres avec tr" << char(130) << "ma (letras con diÈresis):";
	gotoxy(10, 24);
...
void menu_juegos(){
	...
	//Se cargan 100 palabras al array l√≠nea desde un archivo externo
	string linea[105];
	leer_texto(linea);
...
string palabra_aleatoria(string linea[]){
	...
	//Se utiliza la hora en milisegundos en lugar de time(NULL) para evitar demora por la actualizaci√≥n de la semilla
...
//Utiliza un TDA para extraer informaci√≥n de un archivo .txt
...
void ahorcado(string linea[]){
	...
	while(puntos > 30 and !victoria) {
		...
		do{
			...
			if((tolower(guess) < 'a' or tolower(guess) > 'z') and tolower(guess) != 'Ò'){
				gotoxy(35, 5);
				SetConsoleTextAttribute(GetStdHandle (STD_OUTPUT_HANDLE),4);
                cout << "Ingreso incorrecto. Pruebe nuevamente " << endl;
                SetConsoleTextAttribute(GetStdHandle (STD_OUTPUT_HANDLE),7);
			}
		}while((tolower(guess) < 'a' or tolower(guess) > 'z') and tolower(guess) != 'Ò');
...
void palabra_cifrada(string linea[]) {
	...
	gotoxy(45,12);
	SetConsoleTextAttribute(GetStdHandle (STD_OUTPUT_HANDLE),244);
	cout << "AclaraciÛn" << endl;
	Sleep(1000);
	SetConsoleTextAttribute(GetStdHandle (STD_OUTPUT_HANDLE),241);
	gotoxy(10,14);
	cout << "Debido a limitaciones tÈcnicas, se usar· el 4 para referenciar a la Ò en el juego";
	Sleep(5000);
	
	int intentos= 0;
	
	//Definimos la cantidad de intentos de acuerdo al tama√±o de la palabra
	if(tamanio <= 5){
		intentos = tamanio*2;
	}else if(tamanio <= 10){
		intentos = tamanio;
	...
	do{
		...
		//Pasamos la letra a minuscula para evitar que no haya diferencia al ingresar min√∫scula o may√∫scula
		letra= tolower(letra); 
	...
	if(contador == 0){
		...
		cout << "GANASTE :)";
		gotoxy(37, 5);
		cout << "øNo ser·s pariente de Cesar?";
		gotoxy(37, 6);
...
void cifrado(string palabra, int tamano, string &palabra_cifrada) {
	//En este bucle se trabaja con los casos que se dificulta su trabajo por la √± y para la circularidad solo tendremos que evaluar la z, el resto de letras se desplazar√°n una sola unidad
...
void numeros_en_otro_idioma(){
    ...
    lineas(8, 21, 48, '=');
    gotoxy(39, 10);
	cout << "N˙meros en otro idioma" << endl;
	lineas(-1, 11, 28, '=');
    gotoxy(36, 12);
	cout << "1.- EspaÒol";
    gotoxy(36, 13);
...
int magnitud(){
    ...
    cout << "            ";
    gotoxy(20, 6);
    cout << "En quÈ magnitud desea el gr·fico (Magnitudes del 1 al 4): ";
...
void memoria(string linea[]){
    ...
    while(indice < 5){
        ...
        //Si la palabra ya apareci√≥ antes se busca otra
    ...
    gotoxy(9, 6);
    cout << "A continuaciÛn se le mostrar·n 5 palabras por dos segundos, recuerdelas para ganar";
    Sleep(5000);
    lineas(5, 6, 95, ' ');
    ...
    gotoxy(20, 6);
	cout << "a-) " << "øCu·l es la letra que m·s se repitÌa en las palabras?: ";
    letra_repetida = pedir_opciones();
    
    letras_mas_repetidas(palabras, letras_repetidas);

    //Verificamos si el usuario introdujo alguna de las letras que m√°s se repitieron(puede haber m√°s de una)
    unsigned int i = 0;
    while(i < 10){
    ...
    gotoxy(20, 7);
	cout << "b-) " << "øCuantas vocales hay en total?: ";
    ...
    gotoxy(20, 8);
	cout << "c-) " << "øCu·l es la palabra de mayor longitud?: ";
    cin >> palabra_mas_larga;

    palabra_mas_grande(palabras, indices_palabras_mayores, contador);
    i = 0;
    //Si hay mas de una palabra de un largo m√°ximo, el usuario puede introducir cualquiera de las opciones.
    while(i < contador){
    ...
    if(puntaje > 25){
        SetConsoleTextAttribute(GetStdHandle (STD_OUTPUT_HANDLE),250);
        cout << "" << "°Felicitaciones! TenÈs muy buena memoria " << endl;
        SetConsoleTextAttribute(GetStdHandle (STD_OUTPUT_HANDLE),241);
    }
    else{
        SetConsoleTextAttribute(GetStdHandle (STD_OUTPUT_HANDLE),244);
        cout << "Tu memoria no es muy buena. " << "°SeguÌ practicando!" << endl;
        SetConsoleTextAttribute(GetStdHandle (STD_OUTPUT_HANDLE),241);
    }
...
void palabra_mas_grande(string palabras[], int indices_palabras_mayores[], int & contador){
    ...
    //Se busca si hay mas de una palabra del largo m√°ximo
    for(int i = 0; i < 5; i++){
