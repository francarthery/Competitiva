...
//En este caso ademas guarda una de las rutas mas cortas en el vector previous
//Esta ruta se puede reconstruir recorriendo previous desde el fin al inicio
//Si se quiere guardar todas las posibles rutas previous debe ser un vector de vectores de pares
...
vi previous(MAX); //Almacena nodo anterior a i, sirve para reconstruir el camino.
    ...
    while(!Q.empty()){
        ...
        for(it = G[v].begin(); it != G[v].end(); it++){ //Iteramos en los nodos adyacentes a 'v'
            ...
            if(D[v2] > D[v] + cost){ //Nos fijamos si la ruta actual es mas corta que la ya calculada
                ...
                previous[v2] = v; //Almacena anterior nodo en ruta mas corta
            }
        }
    }
}

void ruta(int fin){ //LLamar a ruta en nodo hasta el que queremos saber la ruta desde ini
    if(previous[fin] == 0) cout << fin << " ";
    else{
        ruta(previous[fin]);
        cout << fin << " ";
    }
...
int main(){
    ...
    Dijkstra(ini);
    cout << "La distancia minima es: " << D[fin] << endl;
