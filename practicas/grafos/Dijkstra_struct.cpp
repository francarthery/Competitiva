...
struct Dijkstra {        // WARNING: ii usually needs to be pair<ll, int>
    ...
    void run(int src) {  // O(|E| log |V|)
        ...
        while (sz(Q)) {
            auto [d, node] = Q.top();
            Q.pop();
            if (d > dist[node]) continue; //El caso d == dist[node] lo tenemos que dejar pasar solo una vez
            forall(it, G[node]) if(d + it->fr < dist[it->sc]) { //Ya que le asignamos su distancia antes de procesarlo
                dist[it->sc] = d + it->fr;
                // vp[it->sc] = node; //Para arbol de caminos minimos dividimos al if en el caso < y el caso ==
